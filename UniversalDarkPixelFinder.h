    ...
    static bool findDarkPixelInBip(darkArguments<T> &da){
        ...
        for(int i = startPoint;i<endPoint;i+=da.bands*da.samples){
            ...
            for(int k=0;k<da.areaWidth*da.bands;k+=da.bands){
                ...
                if(channels.count()>2)chR = channels[2]*da.multic;
                if(channels.count()>1)chG = channels[1]*da.multic;
                if(channels.count()>0)chB = channels[0]*da.multic;
...
static void croupImage(QImage &img,QPoint dp,QPoint &croupedPoint){
    ...
    if(deltaTopY<0)deltaTopY = 0;
    else{darkPointY = darkPointY-desSize/2;}

    int deltaBottomY = dp.y()+desSize/2;
    if(deltaBottomY>img.height())deltaBottomY = img.height()-1-deltaTopY;
    else deltaBottomY = desSize;

    int deltaXleft = dp.x()-desSize/2;
    if(deltaXleft<0)deltaXleft = 0;
    else{darkPointX = darkPointX+desSize/2;}

    int deltaXright = dp.x()+desSize/2;
    if(deltaXright>img.width())deltaXright = img.width()-1-deltaXleft;
    else deltaXright = desSize;
