...
ReadHeaderResult EnviReader::loadEnviHeader(EnviHeaderStruct &enviHdr, QString filePath)
{
    ...
    if(!inf.exists()){ return NoFileExists;}
        ...
        while (!in.atEnd())
        {
            ...
            for(int i = 0;i<headerParams.count();++i){
                if(line.contains(headerParams.at(i))&&!line.contains("{")){
                    ...
                    while(!complexParam.contains("}")) complexParam.append(in.readLine());
...
bool EnviReader::loadEnviImageData(QString path)
{
    ...
    if(type==FloatEnvi){emit correctStartSignal(1);setMultiplicator(1);}
    if(type==UnsignIntegerEnvi){emit correctStartSignal(0.006);setMultiplicator(0.006);}
    ...
    *m_byteArray = file->readAll();
    if(type==FloatEnvi)        m_enviImageFloat32 = (float*)m_byteArray->data();
    if(type==UnsignIntegerEnvi)m_enviImageUint16  = (uint16_t*)m_byteArray->data();
...
{
    bool result = false;
    if(isHeaderOk==false||isFileOk==false)return result;
    ...
    if(interLeave == "bip"){
        if(m_enviHeader.dataType == FloatEnvi)  { UniversalImageReader::makeImageForBip(
                        m_Image,
                        channelsSet,
                        loadedPixels,
                        m_enviImageFloat32,
    ...
    if(interLeave == "bil"){
        ...
        if(m_enviHeader.dataType == FloatEnvi)  { UniversalImageReader::makeImageForBil(
                        m_Image,
                        channelsSet,
                        loadedPixels,
                        m_enviImageFloat32,
    ...
    if(interLeave == "bsq"){
        if(m_enviHeader.dataType == FloatEnvi)  {UniversalImageReader::makeImageForBsq(
                        m_Image,
                        channelsSet,
                        loadedPixels,
                        m_enviImageFloat32,
...
void EnviReader::findDarkestPixel(QVector<QRect> *listRect)
{
    ...
    for(int j=0;j<listRect->count();++j){
        ...
        if(m_enviHeader.dataType == FloatEnvi)min = daF.min;
        if(m_enviHeader.dataType == UnsignIntegerEnvi)min = daUint16.min;
    ...
    if(m_enviHeader.dataType == FloatEnvi)darkValues = theDarkestValues;
    if(m_enviHeader.dataType == UnsignIntegerEnvi)darkUint16 = theDarkUint16;
...
void EnviReader::drawAimDarkPointer(QImage img, QPoint darkPoint,QString folderName,QString &file)
{
    ...
    if(!dir.exists())dir.mkdir(checkDir);
...
{
    if(!ignorePolygons.isEmpty())ignorePolygons.clear();
    if(!pointIgnoreList.isEmpty())pointIgnoreList.clear();
    if(lastlistRects)lastlistRects->clear();
}
    ...
    for(int i=0;i<requiredParams.count();++i){
        ...
        for(int j=0;j<paramsList.count();++j){
            ...
            if(paramsList.at(j).contains(requiredParams.at(i))){
                ...
                if(paramsList.at(j).contains("default"))continue;
                ++counter;
            }
...
bool EnviReader::parseParamsHeader(EnviHeaderStruct &enviHdr, QStringList paramsList)
{
    ...
    for(int i = 0;i<paramsList.count();++i){
        ...
        if(paramsList.at(i).contains("sensor type")){continue;}
        if(paramsList.at(i).contains("map info")){continue;}
        if(paramsList.at(i).contains("coordinate system string")){continue;}
        ...
        if(paramsList.at(i).contains("acquisition time")){continue;}
        if(paramsList.at(i).contains("bbl")){continue;}
        if(paramsList.at(i).contains("class lookup")){continue;}
        if(paramsList.at(i).contains("class names")){continue;}
        if(paramsList.at(i).contains("classes")){continue;}
        if(paramsList.at(i).contains("cloud cover")){continue;}
        if(paramsList.at(i).contains("complex function")){continue;}
        if(paramsList.at(i).contains("data ignore value")){continue;}
        if(paramsList.at(i).contains("data offset values")){continue;}
        if(paramsList.at(i).contains("data reflectance gain values")){continue;}
        if(paramsList.at(i).contains("data reflectance offset values")){continue;}
        if(paramsList.at(i).contains("default stretch")){continue;}
        if(paramsList.at(i).contains("dem band")){continue;}
        if(paramsList.at(i).contains("dem file")){continue;}
        if(paramsList.at(i).contains("geo points")){continue;}
        if(paramsList.at(i).contains("pixel size")){continue;}
        if(paramsList.at(i).contains("projection info")){continue;}
        if(paramsList.at(i).contains("read procedures")){continue;}
        if(paramsList.at(i).contains("reflectance scale factor")){continue;}
        if(paramsList.at(i).contains("rpc info")){continue;}
        if(paramsList.at(i).contains("security tag")){continue;}
        if(paramsList.at(i).contains("solar irradiance")){continue;}
        if(paramsList.at(i).contains("spectra names")){continue;}
        if(paramsList.at(i).contains("sun azimuth")){continue;}
        if(paramsList.at(i).contains("sun elevation")){continue;}
        if(paramsList.at(i).contains("x start")){continue;}
        if(paramsList.at(i).contains("y start")){continue;}
        if(paramsList.at(i).contains("z plot average")){continue;}
        if(paramsList.at(i).contains("z plot range")){continue;}
        if(paramsList.at(i).contains("z plot titles")){continue;}
