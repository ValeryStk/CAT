...
QRect EnviModule::findBoundedRect() {
  ...
  for (int i = 0; i < m_polygon->count(); ++i) {
    ...
    if (m_polygon->at(i).p1().x() < topLeftX)
      topLeftX = m_polygon->at(i).p1().x();
    if (m_polygon->at(i).p1().y() < topLeftY)
      topLeftY = m_polygon->at(i).p1().y();
    if (m_polygon->at(i).p1().x() > bottomRightX)
      bottomRightX = m_polygon->at(i).p1().x();
    if (m_polygon->at(i).p1().y() > bottomRightY)
      bottomRightY = m_polygon->at(i).p1().y();
  }

  if (topLeftX > lastPolygonPoint.x())
    topLeftX = lastPolygonPoint.x();
  if (topLeftY > lastPolygonPoint.y())
    topLeftY = lastPolygonPoint.y();
  if (bottomRightX < lastPolygonPoint.x())
    bottomRightX = lastPolygonPoint.x();
  if (bottomRightY < lastPolygonPoint.y())
    bottomRightY = lastPolygonPoint.y();
